...
- (void)queryTopBusinessInfoForTerm:(NSString *)term ll:(NSString*)ll completionHandler:(void (^)(NSDictionary *topBusinessJSON, NSError *error))completionHandler {
    
    NSLog(@"Querying the Search API with term \'%@\' and location \'%@'", term, ll);
    
    //Make a first request to get the search results with the passed term and location
    NSURLRequest *searchRequest = [self _searchRequestWithTerm:term ll:ll];
    ...
    [[session dataTaskWithRequest:searchRequest completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if (!error && httpResponse.statusCode == 200) {
            ...
            #pragma check if any of the businesses are in the businessArray. if its in the array, return it. else return [businessArray firstObject];
...
/**
 ...
 @return The NSURLRequest needed to perform the search
 */
- (NSURLRequest *)_searchRequestWithTerm:(NSString *)term ll:(NSString*)ll{
    NSDictionary *params = @{
                             @"term": term,
                             @"ll": ll,
                             @"limit": kSearchLimit,
